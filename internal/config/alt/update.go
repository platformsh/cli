...
func Update(ctx context.Context, cnf *config.Config, debugLog func(fmt string, i ...any)) error {
	...
	tmpPath := cnf.SourceFile + ".tmp"
	if err := os.WriteFile(tmpPath, b, 0o600); err != nil {
		return err
	}

	oldPath := cnf.SourceFile + ".bak"
	if err := os.Rename(cnf.SourceFile, oldPath); err != nil {
		return err
	}
	debugLog("Backed up config file to: %s", oldPath)

	if err := os.Rename(tmpPath, cnf.SourceFile); err != nil {
		return err
	}
