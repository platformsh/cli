...
func (c *CLIWrapper) copyPHP() error {
	...
	w, err := os.Create(filepath.Join(c.cacheDir(), "php", "php.ini"))
	if err != nil {
		return fmt.Errorf("could not open php.ini file for writing: %w", err)
	}
	defer w.Close()

	tpl := template.Must(template.New("php.ini").Parse(phpIniTemplate))
	if err := tpl.Execute(w, map[string]string{"CLIDir": c.cacheDir()}); err != nil {
		return fmt.Errorf("could not write php.ini file: %w", err)
	}

	if err := os.WriteFile(filepath.Join(c.cacheDir(), "php", "extras", "cacert.pem"), caCert, 0o644); err != nil {
		return err
	}
